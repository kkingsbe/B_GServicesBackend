...
if(!empty($first) || !empty($last) || !empty($email) || !empty($date) || !empty($address) || !empty($notes)) {
 ...
 $sql    = "UPDATE ActiveSchedules SET first = $first, last = $last, email = $email, phone = $phone, date = $date, address = $address, notes = $notes, services = $services, reoccuring = $reoccuring, paymentmethod = $paymentmethod, price = $price WHERE jobid = $jobid";
 if($conn->query($sql)){
